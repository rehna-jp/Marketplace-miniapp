...
```typescript
import { useCastReactions } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useCastReactions(
  hash: string,
...
- **Type**: `string`
- **Required**: Yes
- **Description**: - Cast hash to fetch reactions for
...
- `types: Array<"all" | "likes" | "recasts">` - Reaction types to search for (REQUIRED)
- `viewer_fid?: number` - When provided, returns reactions that respect this user's mutes and blocks and includes `viewer_context`
- `limit?: number` - Results per page (default: 25, max: 100)

### options

- **Type**: `InfiniteQueryHookOptions<ReactionsCastResponse, ReactionForCast>`
- **Required**: No
...
- `data.pages:` Array of `ReactionForCast` with:
  Reaction for cast

Represents a single reaction on a cast with user information.

- `reaction_type:` ReactionType - Type of reaction ('like' or 'recast')
...
- `user:` User - User who made the reaction
...
function MyComponent() {
  const result = useCastReactions("example", /* value */);
...
function CastLikes({ hash }: { hash: string }) {
  ...
  return (
    <div>
      {reactions.map((r) => (
        <div key={r.user.fid}>{r.user.display_name}</div>
      ))}
      {hasNextPage && (
