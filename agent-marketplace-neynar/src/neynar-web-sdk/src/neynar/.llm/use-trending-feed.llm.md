...
```typescript
import { useTrendingFeed } from "@/neynar-web-sdk/neynar";
```
...
- `limit?: number` - Number of results to fetch per page (Default: 10, Maximum: 10)
- `viewer_fid?: number` - FID of viewer for personalized content (respects mutes/blocks, includes viewer_context)
- `time_window?: "1h" | "6h" | "12h" | "24h" | "7d"` - Time window for trending content ('7d' only available for channel feeds)
- `channel_id?: string` - Channel ID to filter trending casts (mutually exclusive with parent_url)
- `parent_url?: string` - Parent URL to filter trending casts (mutually exclusive with channel_id)
- `provider?: "neynar" | "openrank" | "mbd"` - Provider of trending feed
- `provider_metadata?: string` - URI-encoded JSON metadata for provider (only available for 'mbd' provider)
- `x_neynar_experimental?: boolean` - Enable experimental features (sent as global header)
...
function MyComponent() {
  const result = useTrendingFeed({ limit: 123, viewer_fid: 123 });
