...
```typescript
import { useChannelNotifications } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useChannelNotifications(
  fid: number,
  channelIds: string,
...
- **Type**: `string`
- **Required**: Yes
- **Description**: - Channel ID(s) to filter by (comma separated for multiple channels)
...
function MyComponent() {
  const result = useChannelNotifications(123, "example");
...
function ChannelNotifications({
  ...
  const { data, fetchNextPage, hasNextPage } = useChannelNotifications(
    fid,
    channelId,
  );
