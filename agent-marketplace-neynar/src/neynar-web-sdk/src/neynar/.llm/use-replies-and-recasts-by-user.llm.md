...
```typescript
import { useRepliesAndRecastsByUser } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useRepliesAndRecastsByUser(
  fid: number,
...
- `filter?: "replies" | "recasts" | "all"` - Filter to fetch only replies or recasts
- `limit?: number` - Results per page (default: 25, max: 50)
- `viewer_fid?: number` - When provided, returns a feed that respects this user's mutes and blocks and includes `viewer_context`
...
function MyComponent() {
  const result = useRepliesAndRecastsByUser(123, { filter: "replies", limit: 123 });
