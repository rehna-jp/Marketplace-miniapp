...
```typescript
import { useFrameSearch } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useFrameSearch(
  query: string,
...
TanStack Query infinite result with paginated search results
...
function MyComponent() {
  const result = useFrameSearch("example", /* value */);
...
Frame search with infinite scroll

```tsx
const { data, fetchNextPage, hasNextPage, isLoading } = useFrameSearch("game", {
  limit: 20,
});
