...
```typescript
import { useUserByUsername } from "@/neynar-web-sdk/neynar";
```
...
- `viewer_fid?: number` - When provided, adds `viewer_context` to response with relationship status
- `x_neynar_experimental?: boolean` - Enables experimental features including spam score

### options

- **Type**: `QueryHookOptions<UserResponse, User>`
- **Required**: No
...
function MyComponent() {
  const result = useUserByUsername("example", { viewer_fid: 123, x_neynar_experimental: true });
