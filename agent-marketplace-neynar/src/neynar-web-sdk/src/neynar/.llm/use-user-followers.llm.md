...
- **Type**: `number`
- **Required**: Yes
- **Description**: - The Farcaster ID of the user whose followers to fetch
...
- `viewer_fid?: number` - When provided, respects mutes/blocks and adds `viewer_context` to each follower's user object
- `sort_type?: "desc_chron" | "algorithmic"` - Sort order (default: "desc_chron" - most recent first)
- `x_neynar_experimental?: boolean` - Enables experimental features
...
TanStack Query infinite result with paginated follower data
...
function MyComponent() {
  const result = useUserFollowers(123, { viewer_fid: 123, sort_type: "desc_chron" });
