...
```typescript
import { useChannelAndChildFeed } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useChannelAndChildFeed(
  channelId: string,
...
- **Type**: `string`
- **Required**: Yes
- **Description**: - The parent channel identifier to retrieve content from (includes children)
...
- `with_replies?: boolean` - Include replies in response (Default: false)
- `members_only?: boolean` - Only include casts from channel members (Default: true)
- `fids?: string` - Comma separated list of FIDs to filter the feed by (max: 10)
- `should_moderate?: boolean` - Only show casts liked by channel moderator if moderator exists
...
function MyComponent() {
  const result = useChannelAndChildFeed("example", { limit: 123, viewer_fid: 123 });
...
function ChannelHierarchyFeed({ channelId }: { channelId: string }) {
  const { data, fetchNextPage, hasNextPage } = useChannelAndChildFeed(
    channelId,
    {
      viewer_fid: 123,
    },
