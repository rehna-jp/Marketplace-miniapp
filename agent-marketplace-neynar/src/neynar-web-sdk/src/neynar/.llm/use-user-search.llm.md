...
- `viewer_fid?: number` - When provided, adds `viewer_context` to results and respects user's mutes/blocks
- `x_neynar_experimental?: boolean` - Enables experimental features (sent as global header)
- `limit?: number` - Results per page (default: 5, max: 10)

### options

- **Type**: `InfiniteQueryHookOptions<UserSearchResponse, User>`
- **Required**: No
...
TanStack Query infinite result with paginated user search results
...
function MyComponent() {
  const result = useUserSearch("example", { viewer_fid: 123, x_neynar_experimental: true });
