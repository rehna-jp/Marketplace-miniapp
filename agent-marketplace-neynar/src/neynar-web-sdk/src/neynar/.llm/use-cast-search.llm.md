...
```typescript
import { useCastSearch } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useCastSearch(
  query: string,
...
- **Type**: `string`
- **Required**: Yes
- **Description**: - Query string to search for casts (supports operators)
...
- `mode?: "literal" | "semantic" | "hybrid"` - Search mode (default: 'literal')
- `sort_type?: "desc_chron" | "chron" | "algorithmic"` - Sort type (default: 'desc_chron')
- `author_fid?: number` - FID of user whose casts to search
- `viewer_fid?: number` - When provided, returns results that respect this user's mutes and blocks and includes `viewer_context`
- `parent_url?: string` - Parent URL of the casts to search
- `channel_id?: string` - Channel ID of the casts to search
- `limit?: number` - Results per page (default: 25, max: 100)

### options

- **Type**: `InfiniteQueryHookOptions<CastsSearchResponse, Cast>`
- **Required**: No
...
function MyComponent() {
  const result = useCastSearch("example", { x_neynar_experimental: true, mode: "literal" });
...
function CastSearch() {
  ...
  const { data, fetchNextPage, hasNextPage } = useCastSearch(
    q,
    {
      mode: "hybrid",
    },
  ...
  return (
    <div>
      ...
      {casts.map((cast) => (
        <div key={cast.hash}>{cast.text}</div>
      ))}
      {hasNextPage && (
