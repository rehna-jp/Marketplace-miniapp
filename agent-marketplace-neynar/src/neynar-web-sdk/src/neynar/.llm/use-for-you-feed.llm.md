...
```typescript
import { useForYouFeed } from "@/neynar-web-sdk/neynar";
```
...
- **Type**: `number`
- **Required**: Yes
- **Description**: - FID of user whose personalized feed to retrieve
...
- `limit?: number` - Number of results to fetch per page (Default: 25, Maximum: 50)
- `viewer_fid?: number` - FID of viewer for personalized content (respects mutes/blocks, includes viewer_context)
- `provider?: "neynar"` - Provider of the For You feed (only 'neynar' currently supported)
...
function MyComponent() {
  const result = useForYouFeed(123, { limit: 123, viewer_fid: 123 });
