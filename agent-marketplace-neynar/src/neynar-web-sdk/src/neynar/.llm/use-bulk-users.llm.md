...
```typescript
import { useBulkUsers } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useBulkUsers(
  fids: number[],
...
- **Type**: `number[]`
- **Required**: Yes
- **Description**: - Array of Farcaster IDs to fetch (max: 100, auto-truncated if exceeded)
...
- `viewer_fid?: number` - When provided, adds `viewer_context` to each user with relationship status
- `x_neynar_experimental?: boolean` - Enables experimental features including spam score filtering (sent as global header)

### options

- **Type**: `QueryHookOptions<BulkUsersResponse, User[]>`
- **Required**: No
...
TanStack Query result with user data
...
function MyComponent() {
  const result = useBulkUsers(123, { viewer_fid: 123, x_neynar_experimental: true });
...
function UserList({ userFids }: { userFids: number[] }) {
  ...
  return (
    <div>
      {users?.map((user) => (
        <div key={user.fid}>{user.display_name}</div>
      ))}
    </div>
  );
}
