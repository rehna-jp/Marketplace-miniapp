...
```typescript
import { useNotificationsByParentUrl } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useNotificationsByParentUrl(
  fid: number,
  parentUrls: string,
...
- **Type**: `string`
- **Required**: Yes
- **Description**: - Parent URL(s) to filter by (comma separated for multiple URLs)
...
function MyComponent() {
  const result = useNotificationsByParentUrl(123, "example");
...
function FrameNotifications({
  ...
  const { data, fetchNextPage, hasNextPage } = useNotificationsByParentUrl(
    fid,
    frameUrl,
  );
