...
```typescript
import { useMuteList } from "@/neynar-web-sdk/neynar";
```
...
- `limit?: number` - Number of results to fetch
...
- **Type**: `InfiniteQueryHookOptions<MuteListResponse, MuteRecord[]>`
- **Required**: No
...
TanStack Query infinite result with paginated mute data

- `data.pages:` Array of `MuteRecord` with:
  Mute record

Represents a mute relationship between two Farcaster users.

- `object: 'mute'` - Object type identifier (always 'mute')
- `muted:` User - User who is muted
...
function MyComponent() {
  const result = useMuteList(/* value */, []);
