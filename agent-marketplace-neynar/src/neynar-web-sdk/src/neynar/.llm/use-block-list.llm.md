...
```typescript
import { useBlockList } from "@/neynar-web-sdk/neynar";
```
...
- `blocker_fid?: number` - Returns users that this user has blocked

Provide either `blocker_fid` OR `blocked_fid`, not both.

- `blocked_fid?: number` - Returns users that have blocked this user
...
- Default: 20
- Maximum: 100
- `x_neynar_experimental?: boolean` - No description available

### options

- **Type**: `InfiniteQueryHookOptions<BlockListResponse, BlockRecord[]>`
- **Required**: No
...
TanStack Query infinite result with paginated block data

- `data.pages:` Array of `BlockRecord` with:
  Block record

Represents a block relationship between two Farcaster users.

- `object: 'block'` - Object type identifier (always 'block')
- `blocked?:` User - User who is blocked
- `blocker?:` User - User who created the block
...
function MyComponent() {
  const result = useBlockList(/* value */, []);
