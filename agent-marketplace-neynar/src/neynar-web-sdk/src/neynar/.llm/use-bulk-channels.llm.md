...
```typescript
import { useBulkChannels } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useBulkChannels(
  ids: string[],
...
- **Type**: `string[]`
- **Required**: Yes
- **Description**: - Array of channel IDs or parent URLs to fetch (max: 100, auto-truncated if exceeded)
...
- `type?: "id" | "parent_url"` - Type of identifier (default: 'id')
- `viewer_fid?: number` - When provided, adds viewer_context with follow status

### options

- **Type**: `QueryHookOptions<ChannelListResponse, Channel[]>`
- **Required**: No
...
TanStack Query result with channel data
...
function MyComponent() {
  const result = useBulkChannels("example", { type: "id", viewer_fid: 123 });
