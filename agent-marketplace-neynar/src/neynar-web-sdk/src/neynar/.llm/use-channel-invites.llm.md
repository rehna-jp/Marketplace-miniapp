...
```typescript
import { useChannelInvites } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useChannelInvites(
...
- `channel_id?: string` - Channel ID for the channel being queried
- `invited_fid?: number` - FID of the user being invited
...
TanStack Query result with invite data

- `data:` `ChannelMemberInvite` with:
...
function MyComponent() {
  const result = useChannelInvites({ channel_id: "example", invited_fid: 123 });
...
function ChannelInviteManager({ channelId }: { channelId: string }) {
  const {
    ...
    isLoading,
    error,
  } = useChannelInvites({ channel_id: channelId });
  ...
  return (
    <div>
      ...
      <div className="invites-list">
        {invites.map((invite) => (
          <div
            key={`${invite.invited.fid}-${invite.channel_id}`}
            className="invite-item"
          >
            <div className="inviter">
              <img
                src={invite.inviter.pfp_url}
