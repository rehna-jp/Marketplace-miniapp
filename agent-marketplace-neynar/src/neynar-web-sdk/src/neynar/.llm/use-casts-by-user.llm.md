...
- `app_fid?: number` - Optionally filter to casts created via a specific app FID (e.g. 9152 for Warpcast)
- `viewer_fid?: number` - FID of the user viewing the feed (adds viewer context)
- `limit?: number` - Results per page (default: 25, max: 150)
- `include_replies?: boolean` - Include reply casts by the author in the response (default: true)
- `parent_url?: string` - Parent URL to filter the feed (mutually exclusive with channel_id)
- `channel_id?: string` - Channel ID to filter the feed (mutually exclusive with parent_url)

### options

- **Type**: `InfiniteQueryHookOptions<FetchCastQuotes200Response, Cast>`
- **Required**: No
...
function MyComponent() {
  const result = useCastsByUser(123, { x_neynar_experimental: true, app_fid: 123 });
