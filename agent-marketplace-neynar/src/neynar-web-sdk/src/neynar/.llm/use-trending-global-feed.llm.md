...
```typescript
import { useTrendingGlobalFeed } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useTrendingGlobalFeed(
...
- `time_window?: "1h" | "6h" | "12h" | "24h"` - Time window for trending content (do NOT use '7d' - only available for channel feeds)
...
function MyComponent() {
  const result = useTrendingGlobalFeed({ limit: 123, viewer_fid: 123 });
