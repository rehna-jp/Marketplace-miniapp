...
```typescript
import { useChannelFollowers } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useChannelFollowers(
  channelId: string,
...
- `viewer_fid?: number` - When provided, respects mutes/blocks and adds viewer_context
- `limit?: number` - Results per page (default: 25, max: 1000)
...
function MyComponent() {
  const result = useChannelFollowers("example", { x_neynar_experimental: true, viewer_fid: 123 });
