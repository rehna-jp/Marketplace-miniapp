...
```typescript
import { usePopularCastsByUser } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function usePopularCastsByUser(
  fid: number,
...
- **Type**: `number`
- **Required**: Yes
- **Description**: - The FID of the user whose popular casts to fetch
...
- `viewer_fid?: number` - FID of viewing user for personalized context
- `limit?: number` - Maximum number of popular casts to return
...
TanStack Query result containing array of popular casts, loading state, and error info
...
function MyComponent() {
  const result = usePopularCastsByUser(123, { viewer_fid: 123, limit: 123 });
...
User's top casts showcase

```tsx
function UserTopCasts({ fid }: { fid: number }) {
  const { data: topCasts, isLoading } = usePopularCastsByUser(fid, {
    limit: 5,
  });
