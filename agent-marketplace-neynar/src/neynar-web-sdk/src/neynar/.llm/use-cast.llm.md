...
- **Type**: `string`
- **Required**: Yes
- **Description**: - Cast hash or Warpcast URL to fetch
...
- `type: "hash" | "url"` - Identifier type (REQUIRED - specify whether identifier is a hash or URL)
- `viewer_fid?: number` - When provided, adds `viewer_context` to show whether viewer has liked or recasted the cast
...
- **Type**: `QueryHookOptions<CastResponse, Cast>`
- **Required**: No
...
TanStack Query result with cast data
...
function MyComponent() {
  const result = useCast("example", /* value */);
