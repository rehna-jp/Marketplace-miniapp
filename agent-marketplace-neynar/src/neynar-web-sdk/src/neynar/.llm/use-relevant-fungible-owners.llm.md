...
```typescript
import { useRelevantFungibleOwners } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useRelevantFungibleOwners(
...
- `contract_address: string` - Contract address of the fungible token
- `network: "ethereum" | "optimism" | "base" | "arbitrum" | "solana"` - Network of the fungible asset (ethereum, optimism, base, arbitrum, solana)
- `viewer_fid?: number` - When provided, returns token holders from user's network; when omitted, returns top token holders across network

### options

- **Type**: `QueryHookOptions<RelevantFungibleOwnersResponse, User[]>`
- **Required**: No
...
TanStack Query result with relevant owners data
...
function MyComponent() {
  const result = useRelevantFungibleOwners(/* value */, []);
