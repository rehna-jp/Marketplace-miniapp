...
```typescript
import { useCastQuotes } from "@/neynar-web-sdk/neynar";
```

## Hook Signature

```typescript
function useCastQuotes(
  identifier: string,
...
- `viewer_fid?: number` - When provided, adds viewer context
...
function MyComponent() {
  const result = useCastQuotes("example", /* value */);
...
function CastQuotes({ hash }: { hash: string }) {
  ...
  return (
    <div>
      {quotes.map((quote) => (
        <div key={quote.hash}>{quote.text}</div>
      ))}
      {hasNextPage && (
